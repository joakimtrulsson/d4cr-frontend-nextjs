...
// fixa sen resten av principle slug -ie bulletlist och flytta querien, forsätt sen med popupen
export default function PrinciplesPage(props) {

    const principleNumber = props.principleNumbers
    console.log(principleNumber)
    
    //console.log('first', props.principleNumbers[0].number, props.principleNumbers[0].principles.subHeader, props.principleNumbers[0].principles.quote, props.principleNumbers[0].principles.quoteAuthor)
    ...
    const [logoSrc, setLogoSrc] = useState(ButtonDown);
        ...
        const currentIndex = principleNumber.findIndex(numbers => numbers.principles.slug === props.slug);
        const previousIndex = currentIndex - 1;
        const previousSlug = previousIndex >= 0 ? principleNumber[previousIndex].principles.slug : null;
        //nästa slug
        const nextIndex = currentIndex + 1;
        const nextSlug = nextIndex < principleNumber.length ? principleNumber[nextIndex].principles.slug : null;
        ...
        return (
            <RootLayout navMenuData={props.navMenuData} footerMenuData={null} tabTitle={useIndex?.title} resolvedUrl={props.resolvedUrl} language="en_GB">
                <main>
                    ...
                    <div>
                            ...
                            <div className='flex flex-row flex-nowrap title-image-container'>
                                ...
                                <div className="flex flex-row flex-nowrap title-div">
                                    ...
                                    <div className="flex flex-column width--m left-part">
                                        ...
                                        <p className="margin--zero sub-heading-m gray-color">{'Principle ' + principleNumber[currentIndex]?.number}</p>
                                        <h1>{useIndex?.title}</h1>
                                        <p className="padding-tb--xxs margin--zero gray-color">{useIndex?.subHeader}</p>
                                    </div>
                        ...
                        <BottomWave fillColorCode={getColorCode('turquoise-50')} className="bottom-wave" />
                    </div>
                ...
                <LargeBulletList content={contentForLargeBulletList} />
                    ...
                    <> {resources ? (<Resources content={resources} title={principleNumber[currentIndex].principles.resourcesTitle} preamble={principleNumber[currentIndex].principles.resourcesPreamble} />) : (null)}</>
...
// return (
// <RootLayout navMenuData={props.navMenuData} footerMenuData={null} tabTitle={principle.title} resolvedUrl={props.resolvedUrl} language="en_GB">
