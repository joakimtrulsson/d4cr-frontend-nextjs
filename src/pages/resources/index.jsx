...
export default function ResourcesPage(props) {
    ...
    if (!props.navMenuData || (!props.resourcesCat)) { // add footerMenuData here please!
        return notFound();
    }

    return (

        <RootLayout navMenuData={props.navMenuData} footerMenuData={null} tabTitle={title} language="en_GB">
            <RenderResourcesContent resourcesCat={props.resourcesCat} />
        </RootLayout >
    );
}
        ...
        <main className="slug-resources-outer-container flex flex-column flex-align-center">
            ...
            < div className="slug-resources-inner-container  flex flex-row flex-wrap flex-justify-start flex-align-between ">
                {currentItems.map((resource) => (
                    ...
                    <ResourceCard key={resource.id} img={resource.image?.url || null} url={resource.url} title={resource.title} resourceType={resource.resourceType?.type} />
    ...
    try {
        ...
        return {
            props: {
                navMenuData,
                footerMenuData,
                // resolvedUrl can be included if needed for logging or future use.
                resourcesCat // This will contain the resource categories data.
            }
        };
